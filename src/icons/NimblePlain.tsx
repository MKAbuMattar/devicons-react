import * as React from 'react'
import { SVGProps } from 'react'

const SvgNimblePlain = (props: SVGProps<SVGSVGElement>) => (
  <svg width={128} height={128} xmlns="http://www.w3.org/2000/svg" {...props}>
    <path
      style={{
        fill: '#000',
        fillOpacity: 1,
        fillRule: 'nonzero',
        stroke: 'none',
      }}
      d="M2.262 24.707v61.512L64 110.609V45.621Zm18.547 22.656 2.226 3.043c1.528-.254 3.07-.543 4.871-.097l2.297-2.832 2.348 3.695c1.91.246 3.601 1.168 5.328 2.078l2.617-2.371 1.582 4.824c1.68 1.61 2.152 2.422 2.82 3.488l2.473-.886-1.45 7.96-.112.266-3.032 10.985c-2.172 5.593-8.605 6.504-12.574 5.171-3.828-1.285-9.484-5.972-11.258-12.175l-2.367-11.063-.086-.281-1.09-7.43 1.875 1.614c.52-.707.891-1.266 2.23-2.133zm6.238 9.211c-4.266.16-7.344 2.688-8.078 6.074l2.168 4.883 3.074 2.246 5.992-3.39 6.344 6.883 3.558-.497 2.645-4.054c-1.117-5.153-6.477-10.547-12.547-11.852a12.642 12.642 0 0 0-3.156-.293Zm98.691-31.867L64 45.621v64.988l61.738-24.39zm-22.953 7.805.024 16.492s-1.723.75-3.164 2.117c-.954.902-3.454-.148-5.239-.684-.687-.203-2.625 2.051-2.625 2.051V36.23Zm0 0"
      transform="translate(0 5.707)"
    />
    <path
      style={{
        fill: '#000',
        fillOpacity: 1,
        fillRule: 'nonzero',
        stroke: 'none',
      }}
      d="M64.102 4.41 29.988 15.625l11.067-3.629 61.742 20.516 23.043-7.805ZM29.988 15.625l-27.62 9.082 61.734 20.914 27.722-9.39Zm0 0"
      transform="translate(0 5.707)"
    />
  </svg>
)

export default SvgNimblePlain
